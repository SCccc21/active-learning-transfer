...
parser.add_argument('-d', '--dataset', default='mnist2usps', type=str, metavar='DB',
          help='dataset name')
...
parser.add_argument('-s2t', '--src2tgt-rate', default=3, type=float,
...
parser.add_argument('-rd', '--reg_disc', dest='reg_disc', default=0.1, type=float,
          help='regularizer for discriminator (source)')
parser.add_argument('-rent', '--reg-ent', dest='reg_ent', default=1.0, type=float,
          help='regularizer for discriminator (source)')
...
parser.add_argument('--ac', type=str, default='importance_times_entropy_nothres', 
          help='active learning method')
parser.add_argument('--run_name', type=str, default='mnist2usps_dann', 
          help='folder name')
...
def main():
  ...
  if 'usps' in args.dataset:
    from model_usps import Augmentation, data_transformer, load_dbs, create_model
    args.nclasses = 10
  elif 'mnist' in args.dataset:
    from lenet_model import Augmentation, data_transformer, load_dbs, create_model
    args.nclasses = 10
  elif 'stl' in args.dataset:
    from model_stl import Augmentation, data_transformer, load_dbs, create_model
    args.nclasses = 10
  ...
  if args.dataset in ['visda17', 'visda18', 'visda18sbg', 'visda18nbg']:
    ...
    if args.dataset == 'visda17':
      root = '/home/chensi/data/VisDA2017'
    ...
    train_src = dl.ImageFromLMDB(data_path[0], txt_path[0], transform=tf_train)
    train_tgt = dl.ImageFromLMDB(data_path[1], txt_path[1], transform=tf_train)
    test_tgt = dl.ImageFromLMDB(data_path[1], txt_path[2], transform=tf_test)
    train_tgt_noaug = dl.ImageFromLMDB(data_path[1], txt_path[1], transform=tf_test)
  ...
  for round_id in range(start_epoch, args.num_rounds+1):
    ...
    if round_id > 0:
      if args.ac != 'None':
        ...
        train_tgt_noaug = pickle.load(open('./samples/' + args.dataset + '2000_noise.data', 'rb'))

        selected_id = {}
        for sss in range(6):
          selected_id[sss] = select_samples(model_prev, train_tgt_noaug, criterion, sss+round_id*10, args.num_Tl, args.plot_scatter, train_tgt_noaug_selected)
        
        pdb.set_trace()
        pickle.dump(selected_id, open('./rank/'+args.dataset+'_1000_noise.rankcoll', 'wb'))

      print('selected id:',selected_id[3])
      # print('selected labels:',[train_tgt_noaug[i][1] for i in selected_id])
      print('selected labels:',[train_tgt_noaug[1][i] for i in selected_id[3]])
...
def adjust_learning_rate(optimizer, epoch):
  ...
  if 'mnist' or 'stl' in args.dataset:
    if epoch >= 40:
      lr = 0.00005
    elif epoch >= 20:
      lr = 0.0001
