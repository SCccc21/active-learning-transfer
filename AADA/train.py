...
parser.add_argument('--prefix', default='/mnt/nfs/work1/smaji/jcsu/domain-adaptation-pytorch/exp', type=str, metavar='SAVEPATH',
...
parser.add_argument('--num_Tl', default=0, type=int,
          help='# Tl')
...
def main():
  ...
  if args.dataset in ['visda17', 'visda18', 'visda18sbg', 'visda18nbg']:
    ...
    if args.dataset == 'visda17':
      ...
      txt_path = [os.path.join(root, 'train/image_list.txt'), os.path.join(root, 'validation/image_list.txt')]
      lmdb_path = [os.path.join(root, 'train.lmdb'), os.path.join(root, 'validation.lmdb')]
    elif args.dataset == 'visda18sbg':
      root = ROOT[args.dataset]
      data_path = [os.path.join(root, 'train'), os.path.join(root, 'validation')]
      txt_path = [os.path.join(root, 'train/image_list.sbg.txt'), os.path.join(root, 'validation/image_list.nbg.txt')]
      lmdb_path = [os.path.join(root, 'train.sbg.lmdb'), os.path.join(root, 'validation.nbg.lmdb')]
    elif args.dataset == 'visda18nbg':
      root = ROOT[args.dataset]
      data_path = [os.path.join(root, 'train'), os.path.join(root, 'validation')]
      txt_path = [os.path.join(root, 'train/image_list.nbg.txt'), os.path.join(root, 'val_jcsu/image_list.nbg.txt'), \
                  os.path.join(root, 'test_jcsu/image_list.nbg.txt')]
      lmdb_path = [os.path.join(root, 'train.nbg.lmdb'), os.path.join(root, 'val_jcsu.nbg.lmdb'), \
                   os.path.join(root, 'test_jcsu.nbg.lmdb')]
    train_src = dl.ImageFromLMDB(data_path[0], txt_path[0], lmdb_path[0], transform=tf_train)
    train_tgt = dl.ImageFromLMDB(data_path[1], txt_path[1], lmdb_path[1], transform=tf_train)
    test_tgt = dl.ImageFromLMDB(data_path[1], txt_path[2], lmdb_path[2], transform=tf_test)
    train_tgt_noaug = dl.ImageFromLMDB(data_path[1], txt_path[1], lmdb_path[1], transform=tf_test)
  elif 'office' in args.dataset:
    dataset_src, dataset_tgt = args.dataset.split('_')[-1].split('2')
    tf_train = transforms.Compose([
      transforms.Resize([256,256]),
      transforms.RandomCrop(224),
      transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),
    ...
    tf_test = transforms.Compose([
      ...
      transforms.ToTensor(),
      transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
    train_src = OfficeDataset(domain=dataset_src, split='train', transform=tf_train)
    train_tgt = OfficeDataset(domain=dataset_tgt, split='target_train', transform=tf_train)
    test_tgt = OfficeDataset(domain=dataset_tgt, split='target_test', transform=tf_test)
    train_tgt_noaug = OfficeDataset(domain=dataset_tgt, split='target_train', transform=tf_test)
    print(len(train_src))
    print(len(train_tgt))
    print(len(test_tgt))
  else:
  ...
  # If num_rounds == 1, it's DANN case when we only do 1 round with random selection
  ...
  for round_id in range(start_epoch, args.num_rounds+1):
    ...
    ## Select samples
    if round_id > 0:
      if round_id == 1 and 'office' in args.dataset:
        # Randomly select for first round in office
        ...
        # This is the same as DANN random baseline
        np.random.seed(seed=args.seed_for_Tl)
        index = np.random.permutation(len(train_tgt))
        selected_id = index[:args.num_Tl]
    ...
    elif args.num_rounds==0 and args.ac == 'None' and args.num_Tl > 0:
      # DANN random selection baseline
      np.random.seed(seed=args.seed_for_Tl)
      index = np.random.permutation(len(train_tgt))
      selected_id = index[:args.num_Tl]
      print('selected id:',selected_id)
      print('selected labels:',[train_tgt[i][1] for i in selected_id])
    ...
    if round_id >0 or args.num_Tl > 0:
      ...
      if args.ac == 'kcenter' or args.ac == 'diversity' :
        if round_id == 1:
          train_tgt_noaug_selected = data.Subset(train_tgt_noaug, selected_id)
        else:
          train_tgt_noaug_selected = data.ConcatDataset([train_tgt_noaug_selected, data.Subset(train_tgt_noaug, selected_id)])
          print('already selected labels:',[train_tgt_noaug_selected[i][1] for i in range(len(train_tgt_noaug_selected))])
    ...
    for epoch in range(args.start_epoch, args.epochs):
      if args.sample_source:
        ...
        if round_id > 0:
          train_src_selected = data.ConcatDataset([train_src_selected, train_tgt_subset])
      ...
      writer.add_scalar('train/loss_cls', losses_cls, epoch+1)
      writer.add_scalar('train/acc_cls', top1, epoch+1)
      writer.add_scalar('train/loss_disc_src', losses_disc_src, epoch+1)
      writer.add_scalar('train/loss_disc_tgt', losses_disc_tgt, epoch+1)
      writer.add_scalar('train/acc_disc', top1_disc, epoch+1)
      ...
      if args.evaluate and (epoch+1)%10==0:
        ...
        writer.add_scalar('val/loss_cls', losses, epoch+1)
        writer.add_scalar('val/acc', prec1, epoch+1)
...
def select_samples(model, train_tgt, criterion, seed_for_Tl, num_Tl, plot_scatter=False, train_tgt_noaug_selected=None):
  ...
  if num_Tl > len(train_tgt):
    num_Tl = len(train_tgt)
